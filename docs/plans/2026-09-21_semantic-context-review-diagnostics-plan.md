...
- Integrated native UI acceptance passes on 1.106.0 and 1.138.0: real semantic Quick Picks, four review scope/confirmation flows, Problems Ask/Fix, two-file diagnostic selection/send, 2/3 hunk Preview/Apply, one Undo, picker cancellation and reload clearing findings/proposals/old RPC child. Separate Restricted Mode launches pass on both versions. Deterministic RPC responses replace a live provider; no model request or cost is incurred.
- Native UI testing found notification promises holding Apply/review locks until dismissal. Both paths now detach non-modal reporting, with unresolved-notification controller regressions and native assertions requiring completion before dismissal. Final runners also isolate shared application data and Git configuration.
- Final verification passes: 214 tests / 41 files normally, under Git-hook environment and during packaging; whole-repository Biome (zero errors), production audit, diff check, all final Linux native runs and real Pi tool registration. Final VSIX contains 63 entries / 50 runtime modules, includes notification/observer cleanup, excludes development files/dependencies, and preserves the original ignored artifact. SHA-256: `15d3dfb182b6c24edc5c0387d98eef1f84758f8e720d2f476eefdec2fe79e8d0`.
- Final diff review confirms command/API parity, ownership/trust/path/race guards, bounds, disposal, minimum-version declarations, packaging and compatibility. Failed/throwing Apply now immediately releases diagnostic observations, with regression evidence. Three existing development-only dependency advisories and Windows askpass cleanup leftovers remain disclosed, not shipped/runtime failures.
- All implementation and acceptance checks pass without waivers. Retain this plan only until signed commits are pushed and PR #36 is ready for review, then remove it as required.
