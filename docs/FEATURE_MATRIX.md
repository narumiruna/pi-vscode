...
| In-flight instructions | Enter-to-Steer, platform Follow Up shortcut, attachment snapshots, pending counts and recovered drafts | `piRpcClient.ts`, `piRuntime.ts`, `sidebarAttachments.ts`, `sidebarHtml.ts`; deterministic RPC/lifecycle/attachment/keyboard fixtures. Ordinary composer text, text context and images are supported; unsupported/uncertain queue delivery disconnects without replay. Pi queue grouping settings remain native. |
