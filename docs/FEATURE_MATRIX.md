...
| Codex edit and preview changes | Focused editor actions generate virtual-document diffs and require explicit Apply | Implemented in `src/editorActions.ts`. |
...
| Copilot inline chat | `Pi: Inline Edit` and selection smart actions stay in editor flow and preview edits | Implemented with `Ctrl/Cmd+Alt+I`. |
...
| Claude Code editor awareness | Current file, selection, diagnostics, files, images, and terminal context | Implemented. |
...
| Image-aware chat | Bounded PNG, JPEG, GIF, and WebP RPC image payloads | Implemented. |
