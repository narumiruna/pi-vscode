...
| In-flight instructions | Enter-to-Steer, platform Follow Up shortcut, attachment snapshots, Pi-style pending text, delivery-time transcript entries and recovered drafts | `piRpcClient.ts`, `piRuntime.ts`, `sidebar.ts`, `sidebarAttachments.ts`, `sidebarHtml.ts`; deterministic RPC/lifecycle/attachment/keyboard fixtures. Ordinary composer text, text context and images are supported; only Pi's authoritative user-message start promotes a queued item to the transcript, and unsupported/uncertain delivery disconnects without replay. Pi queue grouping settings remain native. |
