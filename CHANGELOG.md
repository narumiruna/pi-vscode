...
- Add **Fix with PiCode** quick fixes for red and yellow diagnostics through VS Code's lightbulb menu, the editor toolbar, and Ctrl/Cmd+I, with contextual diff previews and inline-edit fallback away from diagnostics.
- Support PiCode Inline Edit for both selections and the current line at the cursor as the Ctrl/Cmd+I fallback when no diagnostic is targeted.
- Redesign PiCode Chat around a compact header, native Quick Pick secondary actions, attachment chips, explicit reconnect, cancellation, history-recovery states, serialized submission, retry-safe composer drafts, and narrow Sidebar layouts.
- Route editor context-menu questions and edit requests through the persistent PiCode Chat session for continuous follow-up, with absolute editor-resource context for multi-root workspaces.
...
- Move the PiCode Chat view's default location to the Secondary Sidebar.
- Add a PiCode button to the editor title toolbar for opening Chat.
...
- Add `PiCode: Suggest Next Edit` with whole-file diff preview and stale-safe explicit apply.
...
- Add a dedicated PiCode conversation view in the VS Code Activity Bar.
...
- Add the native `@picode` VS Code Chat participant.
...
- Add `PiCode: Open Chat` and Husky pre-commit verification.

## 0.1.0

- Add an editor context menu for asking PiCode about selected code.
- Add a safe selection-replacement command for PiCode-generated modifications.
