...
| In-flight text | During an ordinary composer request, Enter sends **Steer** after current tool calls. Alt+Enter sends **Follow Up** on macOS/non-WSL Linux; Windows/WSL uses Ctrl+Q. The same shortcut starts immediately while Pi is idle. Buttons remain available. **Inspect Queue** opens the pending-text snapshot; **Clear Queue** recovers cleared text. **Recovered Drafts** appends a selected draft only if the composer revision is unchanged. No attachments or slash commands are queued. |
