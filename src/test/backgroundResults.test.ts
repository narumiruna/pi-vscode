...
test("observed task commits, untracked additions, deletions and duplicate imports preserve unrelated edits", async () =>
  fixture(async (root, task, origin, git) => {
    await writeFile(path.join(task, "modified"), "task commit\n");
    git(task, "add", "modified");
    git(task, "commit", "-qm", "task");
    await rm(path.join(task, "deleted"));
    await writeFile(path.join(task, "untracked"), "added\n");
...
test("stale source/target, dirty buffers, legacy metadata and unsafe selections fail closed", async () =>
  fixture(async (root, task, origin) => {
    ...
    assert.match(
      (await captureBackgroundResult(task, origin)).files.find((file) => file.path === "link")?.skipped ?? "",
      /Symlink/,
    );
    if (process.platform !== "win32") {
      ...
      for (const name of ["modified", "executable"])
        assert.match(modes.files.find((file) => file.path === name)?.skipped ?? "", /Executable-bit/);
    }
  }));

test("partial I/O failure reports exact progress and source remains recoverable", async () =>
  fixture(async (root, task, origin) => {
    await writeFile(path.join(task, "a"), "first");
    await writeFile(path.join(task, "b"), "second");
    const snapshot = await captureBackgroundResult(task, origin);
    const report = await importBackgroundResult(snapshot, ["a", "b"], {
      ...options,
      beforeWrite: async (name) => {
        if (name === "b") throw new Error("simulated I/O failure");
      },
    });
    assert.deepEqual(report.applied, ["a"]);
    assert.match(report.failed[0] ?? "", /b: simulated/);
...
test("Git index modes preserve executable content changes when filesystem modes are not authoritative", async () =>
  fixture(async (root, task, origin, git) => {
    await writeFile(path.join(root, "executable"), "base\n");
    git(root, "add", "executable");
    git(root, "update-index", "--chmod=+x", "executable");
    git(root, "commit", "-qm", "executable base");
    ...
    assert.match(
      (await captureBackgroundResult(task, recorded)).files.find((file) => file.path === "executable")?.skipped ?? "",
      /Executable-bit/,
    );
    await writeFile(path.join(task, "new-executable"), "new\n");
    git(task, "add", "new-executable");
    git(task, "update-index", "--chmod=+x", "new-executable");
    assert.match(
      (await captureBackgroundResult(task, recorded)).files.find((file) => file.path === "new-executable")?.skipped ??
        "",
      /Executable-bit/,
    );
  }));

test("a skipped oversized candidate does not consume the budget for later fitting files", async () =>
  fixture(async (root, task, origin, git) => {
    for (const [name, size] of [
      ...
      await writeFile(path.join(root, name), "a".repeat(size));
    git(root, "add", "a", "b", "c", "d");
    git(root, "commit", "-qm", "bounded base");
    const recorded = { ...origin, baseCommit: git(root, "rev-parse", "HEAD") };
    git(task, "reset", "--hard", recorded.baseCommit);
    for (const [name, size] of [
      ["a", 90_000],
      ...
      await writeFile(path.join(task, name), "b".repeat(size));
    const snapshot = await captureBackgroundResult(task, recorded);
    ...
    assert.equal(
      snapshot.files.reduce(
        (sum, file) => sum + Buffer.byteLength(file.before ?? "") + Buffer.byteLength(file.after ?? ""),
        0,
      ),
      360_020,
    );
...
test("observed Git reads never execute configured filters/fsmonitor or inherited external diff helpers", async () =>
  fixture(async (root, task, origin, git) => {
    const helper = path.join(root, "helper.cjs");
    const marker = path.join(root, "helper-ran");
    await writeFile(
      helper,
      `require('fs').writeFileSync(${JSON.stringify(marker)},'unsafe helper');process.stdout.write('rewritten');`,
    );
