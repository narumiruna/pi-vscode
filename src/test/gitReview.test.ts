...
test("staged snapshots preserve index/worktree and partial staging with unusual names", async () =>
...
test("unborn and empty staging, additions, deleted sides, rename and unsupported files", async () =>
  fixture(async (root, git) => {
      ...
      findings: [
        {
          ...
          endLine: 2,
          severity: "warning",
          message: "Deletion removes required behavior",
        },
      ],
    };
    ...
    for (const patch of [
      ...
      assert.throws(() =>
        parseGitReview(JSON.stringify({ ...valid, findings: [{ ...valid.findings[0], ...patch }] }), snapshot),
      );
    }
    for (const value of ["", "{}", '{"findings":[]}', "x".repeat(100001)])
      assert.throws(() => parseGitReview(value, snapshot));
  }));

test("rejected staged file sides do not consume the retained review budget", async () =>
  fixture(async (root, git) => {
