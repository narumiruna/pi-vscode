...
test("Trash classification is narrow and permanent deletion stays bound to the original session", async () => {
  ...
  try {
    const { isTrashUnavailableError, PiRuntimeManager, runtimeSessionIdentityChanged } =
      require("../piRuntime") as typeof import("../piRuntime");
    ...
    assert.equal(
      runtimeSessionIdentityChanged(originalIdentity, {
    ...
    assert.equal(
      runtimeSessionIdentityChanged(originalIdentity, {}),
      false,
      "a failed restart is not evidence of a replacement session",
    );
    assert.equal(
      isTrashUnavailableError({ code: "Unavailable", message: "Trash unavailable for /remote/session.jsonl" }),
      true,
    );
    assert.equal(
      isTrashUnavailableError({ code: "Unavailable", message: "Remote file system is temporarily unavailable" }),
      false,
    );
    ...
    vscode.workspace.fs = {
      delete: async (uri: { fsPath: string }, options: { useTrash: boolean }) => {
        deletes.push({ path: uri.fsPath, useTrash: options.useTrash });
        rmSync(uri.fsPath);
      },
    };
    ...
    (runtime as any).state = {
      connected: true,
      busy: false,
      sessionFile: path.join(root, "replacement.jsonl"),
      availableModels: [],
...
test("runtime binds permanent deletion to the original Trash-unavailable session", async () => {
  ...
  try {
    ...
    (runtime as any).deleteSessionFile = async (useTrash: boolean) => {
      calls.push({ useTrash });
      throw new SessionTrashUnavailableError("/original/session.jsonl", {
        cause: { code: "Unavailable", message: "remote Trash unsupported /long/internal/path" },
      });
    };
    (runtime as any).deleteSessionPermanently = async (sessionFile: string) => {
      calls.push({ useTrash: false, sessionFile });
    };
    const error = await runtime.deleteSession().then(
      () => undefined,
      (value) => value,
    );
...
test("unsupported Trash keeps the conversation when permanent deletion is cancelled", async () => {
  const vscode = installVscodeMock();
  try {
    const { deleteConversationWithTrashFallback, SessionTrashUnavailableError } =
      require("../piRuntime") as typeof import("../piRuntime");
    ...
    const outcome = await deleteConversationWithTrashFallback({
      ...
      confirmPermanent: async (error) => {
        assert.equal(error.sessionFile, "/original/session.jsonl");
        return false;
      },
...
test("unsupported Trash requires separate confirmation before successful permanent deletion", async () => {
  ...
  try {
    ...
    const outcome = await deleteConversationWithTrashFallback({
      ...
      confirmPermanent: async (error) => {
        calls.push(`confirm:${error.sessionFile}`);
        return true;
      },
      deletePermanently: async (error) => {
        calls.push(`permanent:${error.sessionFile}`);
      },
    });
...
test("permanent and ordinary deletion failures preserve explicit failure outcomes", async () => {
  ...
  try {
    ...
    if (permanent.status === "failed")
      assert.deepEqual(
        { error: permanent.error, permanently: permanent.permanently },
        { error: permanentError, permanently: true },
      );
    ...
    const ordinary = await deleteConversationWithTrashFallback({
      ...
      confirmPermanent: async () => {
        assert.fail("ordinary failures must not offer permanent deletion");
      },
      deletePermanently: async () => {
        assert.fail("ordinary failures must not delete permanently");
      },
    });
    assert.equal(ordinary.status, "failed");
    if (ordinary.status === "failed")
      assert.deepEqual(
        { error: ordinary.error, permanently: ordinary.permanently },
        { error: ordinaryError, permanently: false },
      );
  } finally {
    vscode.restore();
  }
