...
test("Pi history conversion preserves observed text-plus-image user content and merges local labels by stable asset ID", () => {
  ...
  try {
      ...
      [
        {
          ...
          content: [
            {
              type: "text",
              text: "<<<PICODE_CONTEXT_START: src/feature.ts:1-3>>>\ncode\n<<<PICODE_CONTEXT_END>>>\n<<<PICODE_REQUEST_START>>>\nWhat changed?\n<<<PICODE_REQUEST_END>>>",
            },
    ...
    assert.deepEqual(
      first[0]?.attachments?.map((item) => item.type),
      ["context", "image"],
    );
    ...
    assert.deepEqual(
      { label: image.label, width: image.width, height: image.height, availability: image.availability },
      { label: "Image 1", width: 4, height: 5, availability: "available" },
    );

    const known = [
      {
        ...first[0]!,
        attachments: first[0]!.attachments!.map((item) =>
          item.type === "image" ? { ...item, label: "diagram.gif", fullLabel: "screenshots/diagram.gif" } : item,
        ),
      },
    ];
    const refreshed = convertPiMessages(
      [
        {
          role: "user",
          timestamp: 10,
      ...
      { imageAssets: cache, knownMessages: known },
    );
    const refreshedImage = refreshed[0]?.attachments?.[0];
    assert.equal(refreshedImage?.type, "image");
    if (refreshedImage?.type === "image")
      assert.deepEqual(
        { label: refreshedImage.label, fullLabel: refreshedImage.fullLabel },
        { label: "diagram.gif", fullLabel: "screenshots/diagram.gif" },
      );
  } finally {
    vscode.restore();
  }
    ...
    const known = initial.map((message, messageIndex) => ({
      ...
      attachments: message.attachments?.map((attachment) =>
        attachment.type === "image"
          ? {
              ...attachment,
              label: messageIndex === 0 ? "first.gif" : "second.gif",
              fullLabel: messageIndex === 0 ? "one/first.gif" : "two/second.gif",
            }
          : attachment,
      ),
    }));
    const refreshed = convertPiMessages(history, { imageAssets: cache, knownMessages: known });
    const labels = refreshed
      .flatMap((message) => message.attachments ?? [])
      .filter((attachment) => attachment.type === "image")
      .map((attachment) => attachment.fullLabel);
    ...
    const knownSuffix = converted.slice(-2).map((message, index) => ({
      ...
      attachments: message.attachments?.map((attachment) =>
        attachment.type === "image"
          ? {
              ...attachment,
              label: index ? "newest.gif" : "middle.gif",
              fullLabel: index ? "kept/newest.gif" : "kept/middle.gif",
            }
          : attachment,
      ),
    }));
...
test("history conversion bounds the retained suffix before decoding image payloads", () => {
  ...
  try {
    ...
    const history = images.flatMap((bytes, index) => [
      {
        ...
        content: [
          ...
          { type: "image", mimeType: "image/gif", data: bytes.toString("base64") },
        ],
      },
    ...
    assert.deepEqual(
      converted.map((message) => message.content),
      ["message 2", "message 3"],
    );
    assert.deepEqual(
      converted.map((message) => message.id),
      ["picode-user-2-4", "picode-user-3-6"],
      "non-transcript entries do not reduce the retained count or change original indices",
    );
...
test("Pi history conversion deduplicates image payloads and preserves bounded unavailable placeholders", () => {
  ...
  try {
    ...
    assert.equal(
      duplicate[0]?.type === "image" && duplicate[0].assetId,
      duplicate[1]?.type === "image" && duplicate[1].assetId,
    );
    ...
    assert.ok(
      unavailable.every(
        (item) =>
          item.type === "image" && item.availability === "unavailable" && item.assetId.startsWith("unavailable-"),
      ),
    );
  } finally {
    vscode.restore();
...
test("history conversion marks payloads evicted by the bounded cache unavailable", () => {
  ...
  try {
    ...
    assert.equal(
      converted[0]?.attachments?.[0]?.type === "image" && converted[0].attachments[0].availability,
      "unavailable",
    );
    assert.equal(
      converted[1]?.attachments?.[0]?.type === "image" && converted[1].attachments[0].availability,
      "available",
    );
  } finally {
    vscode.restore();
