...
test("recent Pi sessions are workspace-scoped, newest-first, and use names before prompts", async () => {
  ...
  try {
        ...
        { type: "session", version: 3, id: "older-id", cwd: workspace },
        {
          ...
          message: {
            ...
            content: [
              {
                type: "text",
                text: "<<<PICODE_CONTEXT_START: src/sidebar.ts>>>\nconst context = true;\n<<<PICODE_CONTEXT_END>>>\n<<<PICODE_REQUEST_START>>>\nFix the sidebar\n<<<PICODE_REQUEST_END>>>",
              },
            ],
          },
        },
    ...
    assert.ok(result);
    assert.deepEqual(
      result.map((item) => ({ id: item.sessionId, title: item.title })),
      [
  ...
  try {
    ...
    await Promise.all(
      Array.from({ length: 1_024 }, (_, index) =>
        writeFile(path.join(sessions, `empty-${String(index).padStart(4, "0")}.jsonl`), ""),
      ),
    );
    ...
    await writeFile(
      ...
      session([
        { type: "session", version: 3, id: "named-id", cwd: workspace },
        {
          ...
          id: "1",
          parentId: null,
          message: { role: "user", content: [{ type: "text", text: "Initial prompt" }, image("a")] },
        },
        { type: "session_info", id: "2", parentId: "1", name: "Middle name" },
        {
          ...
          id: "3",
          parentId: "2",
          message: { role: "user", content: [{ type: "text", text: "Later prompt" }, image("b")] },
        },
      ]),
    );
    ...
    await writeFile(
      ...
      session([
        ...
        { type: "session_info", id: "2", parentId: "1", name: "Out of budget" },
        {
          ...
          id: "3",
          parentId: "2",
          message: { role: "user", content: [{ type: "text", text: "Later prompt" }, image("c"), image("d")] },
        },
      ]),
    );
