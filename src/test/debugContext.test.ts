...
test("debug capture uses only bounded stack/scopes/local variables, omitting lazy and environment containers", async () => {
  ...
  tracker.observe("s", { type: "event", event: "stopped" });
  ...
  const snapshot = await captureDebug({
    ...
    frameId: 1,
    threadId: 1,
    assertCurrent: () => tracker.assert("s", generation),
    request: async (command) => {
      calls.push(command);
      if (command === "stackTrace")
        return {
          stackFrames: Array.from({ length: 100 }, (_, index) => ({
            id: index + 1,
            name: "frame",
            line: 1,
            source: { path: "/repo/a.js" },
          })),
          totalFrames: 100,
        };
      ...
      return {
        variables: [
          { name: "lazy", value: "unsafe", presentationHint: { lazy: true } },
  ...
  assert.ok(snapshot.frames.length <= 20);
  assert.ok(snapshot.variables.length <= 50);
  assert.ok(
    snapshot.variables.every(
      (variable) => variable.value.length <= 1000 && variable.name !== "lazy" && variable.name !== "env",
    ),
  );
  assert.ok(JSON.stringify(snapshot).length <= 20_000);
  assert.equal(supportedDebugAdapter("pwa-node"), true);
  assert.equal(supportedDebugAdapter("unknown"), false);
});
...
test("debug responses become invalid after continue/step/termination or changed frame; timeout and malformed responses fail", async () => {
  ...
  for (const message of [
    ...
    { type: "request", command: "restart" },
    { type: "event", event: "terminated" },
  ]) {
    tracker.change("s", true);
    const generation = tracker.token("s");
  ...
  await assert.rejects(
    captureDebug({ ...options, request: async () => new Promise(() => {}), timeoutMs: 10 }),
    /deadline/,
  );
  await assert.rejects(
    captureDebug({ ...options, request: async () => ({ stackFrames: [{ id: 1, name: {}, line: -1 }] }) }),
    /Malformed/,
  );
