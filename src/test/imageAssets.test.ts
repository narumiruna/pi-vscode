...
test("image assets validate MIME, canonical Base64, decoded size, format, and dimensions", () => {
  ...
  assert.deepEqual(
    ...
    { mimeType: "image/gif", width: 2, height: 3 },
  );
  ...
  assert.equal(
    assets.store("image/png", bytes.toString("base64")),
    undefined,
    "declared MIME must match a supported image header",
  );
  ...
  assert.ok(
    assets.store("image/png", maximumDimensions.toString("base64")),
    "the documented 4096-square boundary remains available",
  );
  ...
  assert.equal(
    assets.store("image/png", oversizedDimensions.toString("base64")),
    undefined,
    "images above the decoded-pixel budget are rejected",
  );
});
...
test("browser-rejected assets remain blocked across later cache stores", () => {
  ...
  assets.reject(first.id);
  assert.equal(
    assets.isRejected(first.id),
    true,
    "rejections remain recorded even after their payload has left the cache",
  );
  ...
  assert.equal(
    assets.store("image/gif", lateBytes.toString("base64")),
    undefined,
    "a late browser rejection still blocks re-admission after host eviction",
  );
});
...
test("one-shot delivery deduplicates state updates, skips evicted payloads, retries failed posts, and resets for a recreated webview", () => {
  ...
  assert.deepEqual(
    delivery.pending([first.id, first.id], assets).map((asset) => asset.id),
    [first.id],
  );
  assert.deepEqual(delivery.pending([first.id], assets), []);
  delivery.retry(first.id);
  assert.deepEqual(
    delivery.pending([first.id], assets).map((asset) => asset.id),
    [first.id],
  );
  const second = assets.store("image/gif", gif(2, 2, 2).toString("base64"))!;
  assert.deepEqual(
    delivery.pending([first.id, second.id], assets).map((asset) => asset.id),
    [second.id],
  );
  delivery.reset();
  assert.deepEqual(
    delivery.pending([second.id], assets).map((asset) => asset.id),
    [second.id],
  );
});
