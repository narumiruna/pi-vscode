...
function ui(root: string) {
  ...
  vscode.window.showTextDocument = async (document: any) => {
    if (document.uri && providers.has(document.uri.scheme))
      inspected.push(providers.get(document.uri.scheme).provideTextDocumentContent(document.uri));
  };
...
test("snapshot inspection formats zero, one and multiple warnings without exposing content in the prompt", async () => {
  ...
  const disclosure =
    "Inspect this local snapshot. Pi history/instructions and later tool reads are separate. Secret warnings are best-effort. Nothing has been sent yet.";
  try {
...
test("snapshot confirmation preserves cancellation, redaction reinspection and failure limits", async () => {
  ...
  try {
    ...
    const inspected = fixture.inspected.length;
    const prompted = confirmations;
  ...
  try {
    ...
    registerGitReview(fixture.context, fixture.runtime, {
      sendRequest: async (_request, contexts, options) => {
        requests.push({ contexts, options });
        const response = JSON.stringify({
          incomplete: false,
          findings: [
            { path: "a.ts", side: "after", startLine: 1, endLine: 1, severity: "warning", message: "Finding" },
          ],
        });
    ...
    assert.equal(requests[0].options.policy, "read-only");
    assert.match(requests[0].contexts[0].content, /staged/);
    assert.doesNotMatch(requests[0].contexts[0].content, /unstaged/);
    ...
    assert.equal(diffs[0][0], "vscode.diff");
    assert.notEqual(diffs[0][1].scheme, "file");
    assert.notEqual(diffs[0][2].scheme, "file");
    vscode.window.showWarningMessage = async () => {
      git("add", "a.ts");
      return "Send Staged Review";
    };
    await command();
    assert.equal(requests.length, 1);
    assert.match(fixture.errors.pop() ?? "", /stale/);
    vscode.workspace.isTrusted = false;
    await command();
    assert.match(fixture.errors.pop() ?? "", /trusted/);
    vscode.workspace.isTrusted = true;
    vscode.workspace.workspaceFolders = [{ uri: new MockUri("virtual", "/repo"), name: "Virtual" }];
    await command();
    assert.match(fixture.errors.pop() ?? "", /virtual/);
  } finally {
    fixture.dispose();
...
for (const workspaceKind of ["root", "subfolder"])
  test(`staged ${workspaceKind} review preserves runtime scope and highlights complete finding lines`, async () => {
    const root = await mkdtemp(path.join(tmpdir(), "picode-staged-scope-"));
    ...
    registerGitReview(fixture.context, fixture.runtime, {
      sendRequest: async (_request, _contexts, options) => {
        ...
        const response = JSON.stringify({
          incomplete: false,
          findings: [{ ...finding, severity: "warning", message: "Finding" }],
        });
        await options?.onResponse?.(response);
        return response;
      },
    ...
    try {
      ...
      for (const side of ["before", "after"])
        for (const startLine of [1, 2]) {
          finding = { path: "a.ts", side, startLine, endLine: 2 };
          await vscode.registrations.get("picode.reviewStagedChanges")!();
          await vscode.registrations.get("picode.showStagedFindings")!();
          assert.deepEqual(fixture.errors, []);
          assert.equal(selection.startLine, startLine - 1);
          assert.equal(selection.startCharacter, 0);
          assert.equal(selection.endLine, 1);
          assert.equal(selection.endCharacter, side === "before" ? 4 : 7);
        }
      fixture.runtime.currentCwd = workspaceKind === "root" ? nested : root;
      await vscode.registrations.get("picode.showStagedFindings")!();
      assert.match(fixture.errors.pop() ?? "", /target does not match/);
      fixture.runtime.currentCwd = fixture.folder.uri.fsPath;
      fixture.runtime.currentState.sessionId = "changed";
      await vscode.registrations.get("picode.showStagedFindings")!();
...
test("failed-test repair runs a real failing Node test, applies a previewed fixture response, and reruns the unchanged command", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(root, "failure.test.cjs"),
      "const fs=require('node:fs');const n=fs.existsSync('runs')?Number(fs.readFileSync('runs')):0;fs.writeFileSync('runs',String(n+1));require('node:assert/strict').equal(require('./source.cjs'),42);\n",
    );
    ...
    registerTestRepair(fixture.context, fixture.runtime, {
      ...
      addEditProposal: (input) => {
        void (async () => {
          ...
          await input.onPreview(ids);
          previews++;
          await input.onApply(ids);
          applications++;
...
test("test repair rejects dirty sources and buffer/disk changes around observed runs", async () => {
  ...
  vscode.window.showInputBox = async () =>
    JSON.stringify({
      ...
      args: ["-e", "require('fs').writeFileSync('runs','ran');process.exit(1)"],
    });
  vscode.window.showQuickPick = async () => "Run Approved Command";
  vscode.window.showWarningMessage = async (_message: string, ...args: any[]) => {
    if (scenario === "approval-edit") {
      text = "changed";
      version++;
    }
...
test("supplied failure log files preserve multiline evidence and reject missing, binary or oversized input", async () => {
  ...
  const output = "AssertionError: expected 42\r\n  at test.js:3\r\n\n漢字 context\n";
  let picks = 0;
  let cancelLog = false;
  let requests = 0;
  const document = {
  ...
  vscode.window.showOpenDialog = async () =>
    ++picks % 2 === 1 ? [MockUri.file(source)] : cancelLog ? undefined : [MockUri.file(log)];
  ...
  registerTestRepair(fixture.context, fixture.runtime, {
    sendRequest: async (_request, contexts) => {
      ...
      assert.equal(snapshot.status, "supplied-log");
  ...
  try {
    ...
    for (const invalid of [Buffer.from([0, 1]), Buffer.alloc(100_001, 65)]) {
      await writeFile(log, invalid);
      await command();
      assert.equal(requests, 1);
      assert.match(fixture.errors.pop() ?? "", /Binary|Oversized/);
    }
    await rm(log);
    await command();
    assert.match(fixture.errors.pop() ?? "", /no longer exists/);
    cancelLog = true;
    await command();
    assert.equal(requests, 1);
    assert.deepEqual(fixture.errors, []);
...
test("debug capture cancellation, unsupported adapters and resume during inspection never submit to Pi", async () => {
  ...
  vscode.debug = {
    registerDebugAdapterTrackerFactory: (_type: string, value: any) => {
      factory = value;
      return { dispose() {} };
    },
  ...
  const session: any = {
    id: "debug",
    type: "pwa-node",
    workspaceFolder: fixture.folder,
    configuration: {},
    customRequest: async (command: string) => {
      dap++;
      return command === "stackTrace"
        ? { stackFrames: [{ id: 1, name: "paused", line: 1 }] }
        : command === "scopes"
          ? { scopes: [] }
          : { variables: [] };
    },
  ...
  registerDebugContext(fixture.context, fixture.runtime, {
    sendRequest: async () => {
      provider++;
      return "answer";
    },
  ...
  try {
    ...
    await command();
    assert.equal(dap, 0);
    assert.match(fixture.errors.pop() ?? "", /unsupported/);
    ...
    vscode.workspace.getConfiguration = (section: string, scope: any) => ({
      get: (_key: string, fallback: unknown) => {
        ...
        assert.equal(
          scope,
          fixture.folder.uri,
          "resolve settings in the debug session's workspace, not the default root",
        );
    ...
    await command();
    assert.equal(dap, 0);
    assert.match(fixture.errors.pop() ?? "", /getter expansion/);
    expandGetters = false;
    vscode.window.showWarningMessage = async () => {
      expandGetters = true;
      return "Capture Locally";
    };
    await command();
    assert.equal(dap, 0);
    assert.match(fixture.errors.pop() ?? "", /getter expansion/);
    ...
    assert.ok(dap > 0);
    assert.equal(provider, 0);
    assert.match(fixture.errors.pop() ?? "", /paused|changed/);
  } finally {
    fixture.dispose();
    await rm(root, { recursive: true, force: true });
  }
