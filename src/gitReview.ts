...
export interface GitReviewResult {
  readonly findings: readonly GitFinding[];
...
export function reviewContext(snapshot: GitReviewSnapshot): string {
  ...
  if (context.length > 1_000_000)
    throw new Error("Git review context exceeds its transmission limit. Reduce the selected scope.");
  return context;
}
export function parseGitReview(response: string, snapshot: GitReviewSnapshot): GitReviewResult {
  ...
  const findings = result.findings.map((item: unknown): GitFinding => {
    if (
      ...
      !["before", "after"].includes(String(item.side)) ||
    ...
    const file = snapshot.files.find(
      (file) => !file.skipped && (side === "before" ? file.oldPath : file.path) === item.path,
    );
    ...
    return {
      ...
      severity: item.severity as GitFinding["severity"],
