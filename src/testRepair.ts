...
export function validateTestCommand(value: unknown): TestCommand {
  ...
  if (
    ...
    !command.executable ||
    command.executable.length > 2_000 ||
    ...
    command.args.some((arg) => typeof arg !== "string" || arg.includes("\0") || arg.length > 10_000) ||
    JSON.stringify(command.args).length > 20_000
  )
    throw new Error("Use an explicit executable and bounded string arguments, not a shell expression.");
...
export function noTestsMatched(output: string): boolean {
  // biome-ignore lint/suspicious/noControlCharactersInRegex: ANSI color sequences begin with the ESC control character.
...
export class RepairAttempts {
  ...
  public constructor(initialFailure: string) {
    this.previous = digest(initialFailure);
  }
  public approve(): void {
    if (this.stopped || this.attempts >= 2)
      throw new Error("Test repair is stopped or its two-attempt limit is exhausted.");
    this.attempts++;
  }
  public observe(exitCode: number | null, output: string, status: ProcessResult["status"]): void {
    ...
    if (noTestsMatched(output)) {
      this.stopped = "No tests matched";
      return;
    }
    if (exitCode === 0) {
      this.stopped = "Passed";
