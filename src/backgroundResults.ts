...
import { decodeText, digest, git, gitIdentity, gitRevision, gitText, safeRelativePath, validGitOid, type GitIdentity } from "./gitSnapshots";

export interface TaskOrigin { readonly repository: GitIdentity; readonly baseCommit: string }
export interface ResultFile { readonly path: string; readonly before?: string; readonly after?: string; readonly skipped?: string }
export interface BackgroundResult {
  readonly source: GitIdentity; readonly origin: TaskOrigin; readonly files: readonly ResultFile[];
  readonly identity: string; readonly capturedAt: number;
}
export interface ImportReport { readonly applied: string[]; readonly skipped: string[]; readonly failed: string[] }
export function validTaskOrigin(value: unknown): value is TaskOrigin {
  if (!value || typeof value !== "object") return false;
  const item = value as TaskOrigin;
  return typeof item.baseCommit === "string" && validGitOid(item.baseCommit) && !!item.repository
    && [item.repository.root, item.repository.gitDir, item.repository.commonDir].every(v => typeof v === "string" && path.isAbsolute(v) && !v.includes("\0"));
}
export async function assertSafeFile(root: string, relative: string): Promise<string> {
  ...
  for (const [index, part] of relative.split("/").entries()) {
    ...
    try {
      ...
      if (stat.isSymbolicLink() || (index < relative.split("/").length - 1 ? !stat.isDirectory() : !stat.isFile())) throw new Error("Symlink or unsupported file type.");
  ...
  try {
    ...
    try {
      ...
      if (bytesRead !== stat.size || after.size !== stat.size || after.mtimeMs !== stat.mtimeMs || after.ctimeMs !== stat.ctimeMs
        || named.dev !== stat.dev || named.ino !== stat.ino || named.isSymbolicLink()) throw new Error("File changed during snapshot capture.");
...
export async function captureBackgroundResult(sourceRoot: string, origin: TaskOrigin, signal?: AbortSignal): Promise<BackgroundResult> {
  const deadline = AbortSignal.timeout(45_000);
  signal = signal ? AbortSignal.any([signal, deadline]) : deadline;
  if (!validTaskOrigin(origin)) throw new Error("Verified task origin/base metadata is unavailable. Open Source Control instead.");
  const source = await gitIdentity(sourceRoot, signal);
  if (source.commonDir !== origin.repository.commonDir || source.root === origin.repository.root) throw new Error("This task is not an isolated worktree of the recorded repository.");
  ...
  for (const name of [...new Set([...changed, ...untracked].filter(Boolean))].sort()) {
    try {
      ...
      if (entry && !["100644", "100755"].includes(entry.mode)) throw new Error("Unsupported base mode (symlink/submodule).");
  ...
  if (JSON.stringify(await gitRevision(source.root, signal)) !== JSON.stringify(revision)) throw new Error("Task Git state changed during capture.");
  // Detect writes occurring during collection, not only index changes.
  for (const file of files.filter(file => !file.skipped)) if (await readRegularText(source.root, file.path) !== file.after) throw new Error("Task files changed during capture.");
...
export async function importBackgroundResult(snapshot: BackgroundResult, selected: readonly string[], options: {
  assertInactive: () => void; isDirty: (absolutePath: string) => boolean;
  beforeWrite?: (file: string) => Promise<void>;
}): Promise<ImportReport> {
  options.assertInactive();
  if (!selected.length || new Set(selected).size !== selected.length || selected.some(name => !snapshot.files.some(file => !file.skipped && file.path === name))) throw new Error("Invalid selected result files.");
  ...
  if (JSON.stringify(target) !== JSON.stringify(snapshot.origin.repository)) throw new Error("Target repository identity changed.");
  ...
  for (const file of files) {
    try {
      ...
      if (await validate(file) === file.after) { report.skipped.push(file.path); continue; }
      if (await readRegularText(snapshot.source.root, file.path) !== file.after) throw new Error("Source changed after Preview.");
      const absolute = await assertSafeFile(target.root, file.path);
      if (file.after === undefined) await unlink(absolute);
      else {
        await mkdir(path.dirname(absolute), { recursive: true });
        await validate(file);
        const handle = await open(absolute, constants.O_RDWR | constants.O_NOFOLLOW | (file.before === undefined ? constants.O_CREAT | constants.O_EXCL : 0), 0o644);
        try {
          await assertSafeFile(target.root, file.path);
          const opened = await handle.stat(), named = await lstat(absolute);
          if (opened.ino !== named.ino || opened.dev !== named.dev || opened.nlink !== 1 || options.isDirty(absolute)) throw new Error("Destination identity changed while opening.");
          if (file.before !== undefined) {
            ...
            if (original.subarray(0, bytesRead).toString("utf8") !== file.before) throw new Error("Destination changed before writing.");
          }
