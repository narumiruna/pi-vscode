...
export type GitReviewScope =
  ...
  | { readonly kind: "branch"; readonly baseRef: string; readonly baseOid?: string; readonly mergeBase?: string; readonly headOid?: string };
export interface GitReviewSnapshot {
  readonly repository: GitIdentity; readonly revision: GitRevision; readonly scope: GitReviewScope;
  readonly files: readonly GitSnapshotFile[]; readonly diff: string; readonly capturedAt: number; readonly identity: string;
}
export type StagedSnapshot = GitReviewSnapshot;
export interface GitReviewBase { readonly ref: string; readonly oid: string; readonly label: string }
...
export async function captureStaged(cwd: string, signal?: AbortSignal): Promise<StagedSnapshot> {
  ...
  for (let index = 0; index < records.length - 1;) {
    ...
    if (!skipped) {
      for (const oid of [beforeOid, afterOid]) {
        ...
        if (!Number.isSafeInteger(size) || size < 0 || size > Math.min(maxGitReviewFileBytes, remaining - fileBytes)) { skipped = "Oversized file or context limit"; break; }
  ...
  if (reviewed.length) {
    try {
      diff = (await git(repository.root, ["diff", "--cached", "--no-ext-diff", "--no-textconv", "--no-color", "--find-renames", "--", ...new Set(reviewed.flatMap(file => [file.oldPath, file.path]))], signal, maxGitReviewDiffBytes)).toString("utf8");
    } catch (error) {
...
export async function listGitReviewBases(cwd: string, signal?: AbortSignal): Promise<GitReviewBase[]> {
  const repository = await gitIdentity(cwd, signal);
  const fields = gitText(await git(repository.root, ["for-each-ref", "--format=%(refname)%00%(objectname)%00", "refs/heads", "refs/remotes"], signal, 512_000)).split("\0");
...
export async function captureGitReview(cwd: string, requested: GitReviewScope, signal?: AbortSignal, validate = true): Promise<GitReviewSnapshot> {
  ...
  if (requested.kind === "unstaged") {
    ...
    const selected = bases.find(base => base.ref === requested.baseRef && (!requested.baseOid || base.oid === requested.baseOid));
    ...
    names = [...new Set([...before.keys(), ...afterTree.keys()])].filter(name => before.get(name)?.oid !== afterTree.get(name)?.oid || before.get(name)?.mode !== afterTree.get(name)?.mode);
    scope = { kind: "branch", baseRef: selected.ref, baseOid: selected.oid, mergeBase, headOid: revision.head };
    const snapshot = await captureNamedFiles(repository, revision, scope, names, before, name => readTreeText(repository.root, afterTree.get(name), signal), ["diff", "--no-ext-diff", "--no-textconv", "--no-color", "--no-renames", mergeBase, revision.head, "--"], signal);
    if (validate) await assertGitReviewCurrent(snapshot, signal);
    return snapshot;
  }
  const untracked = gitText(await git(repository.root, ["ls-files", "--others", "--exclude-standard", "-z"], signal)).split("\0").filter(Boolean);
  names = [...new Set([...names, ...untracked])].sort();
  if (!revision.head && requested.kind === "workingTree") names = [...new Set([...names, ...(await trackedNames(repository.root, signal)), ...untracked])].sort();
  const snapshot = await captureNamedFiles(repository, revision, scope, names, before, name => readWorktreeText(repository.root, name), diffArgs, signal);
  if (validate) await assertGitReviewCurrent(snapshot, signal);
  return snapshot;
}
...
export async function assertGitReviewCurrent(snapshot: GitReviewSnapshot, signal?: AbortSignal): Promise<void> {
  ...
  if (refreshed.identity !== snapshot.identity) throw new Error(`${scopeLabel(snapshot.scope)} review is stale: Git state or retained files changed. Capture a fresh review.`);
}
...
interface TreeEntry { readonly mode: string; readonly oid: string }
...
async function captureNamedFiles(
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (entry && !["100644", "100755"].includes(entry.mode)) throw new Error("Symlink, submodule, or unsupported file mode");
      ...
      if (Buffer.byteLength(before ?? "") > maxGitReviewFileBytes || Buffer.byteLength(after ?? "") > maxGitReviewFileBytes || bytes > remaining) throw new Error("Oversized file or context limit");
      remaining -= bytes;
    } catch (error) {
      if (signal?.aborted) throw error;
      skipped = error instanceof Error ? error.message : "Unavailable";
    }
    files.push({ path: name, oldPath: name, status: before === undefined ? "A" : after === undefined ? "D" : "M", ...(skipped ? { skipped } : { before, after }) });
  }
  let diff = "";
  if (diffArgs.length && files.some(file => !file.skipped)) {
    try { diff = gitText(await git(repository.root, [...diffArgs, ...files.filter(file => !file.skipped).map(file => file.path)], signal, maxGitReviewDiffBytes)); }
    catch (error) { if (signal?.aborted) throw error; diff = "Diff omitted: output limit or Git diff error. Compare the supplied immutable before/after blobs instead."; }
  }
  ...
  const identity = digest(JSON.stringify({ repository, revision: scope.kind === "branch" ? { head: revision.head } : revision, scope, names, files, diff }));
  return { ...core, identity };
}

async function changedWorktreeNames(root: string, prefix: readonly string[], signal?: AbortSignal): Promise<string[]> {
  return gitText(await git(root, ["diff", "--name-only", "-z", "--no-renames", "--no-ext-diff", "--no-textconv", ...prefix, "--"], signal)).split("\0").filter(Boolean);
}

async function trackedNames(root: string, signal?: AbortSignal): Promise<string[]> {
  return gitText(await git(root, ["ls-files", "-z"], signal)).split("\0").filter(Boolean);
}
...
async function indexTree(root: string, signal?: AbortSignal): Promise<Map<string, TreeEntry>> {
  ...
  for (const record of gitText(await git(root, ["ls-files", "--stage", "-z"], signal)).split("\0").filter(Boolean)) {
...
async function commitTree(root: string, commit: string, signal?: AbortSignal): Promise<Map<string, TreeEntry>> {
  ...
  const result = new Map<string, TreeEntry>();
  for (const record of gitText(await git(root, ["ls-tree", "-r", "-z", commit], signal)).split("\0").filter(Boolean)) {
...
async function readTreeText(root: string, entry: TreeEntry | undefined, signal?: AbortSignal): Promise<string | undefined> {
  ...
  if (!Number.isSafeInteger(size) || size < 0 || size > maxGitReviewFileBytes) throw new Error("Oversized file or context limit");
...
async function readWorktreeText(root: string, relative: string): Promise<string | undefined> {
  ...
  for (const part of relative.split("/").slice(0, -1)) {
    ...
    try { const info = await lstat(parent); if (info.isSymbolicLink() || !info.isDirectory()) throw new Error("Unsupported worktree parent"); }
  ...
  try {
    ...
    if (namedBefore.isSymbolicLink() || !namedBefore.isFile() || namedBefore.nlink !== 1 || namedBefore.size > maxGitReviewFileBytes) throw new Error("Oversized or unsupported worktree file");
    handle = await open(absolute, constants.O_RDONLY | constants.O_NOFOLLOW);
    const opened = await handle.stat();
    if (opened.dev !== namedBefore.dev || opened.ino !== namedBefore.ino || opened.nlink !== 1) throw new Error("Worktree file identity changed during capture");
    const bytes = Buffer.alloc(opened.size + 1);
    const { bytesRead } = await handle.read(bytes, 0, bytes.length, 0);
    const after = await handle.stat();
    const namedAfter = await lstat(absolute);
    if (namedAfter.isSymbolicLink() || namedAfter.dev !== opened.dev || namedAfter.ino !== opened.ino || bytesRead !== opened.size || after.size !== opened.size || after.mtimeMs !== opened.mtimeMs || after.ctimeMs !== opened.ctimeMs) throw new Error("Worktree file changed during capture");
