...
import { type ProcessResult, runBoundedProcess } from "./boundedProcess";
...
export function registerTestRepair(
  ...
  workflowCommand(context, "picode.repairFailedTest", async () => {
    ...
    try {
      ...
      const value = await vscode.window.showInputBox({
        title: "Approved test command (JSON)",
        prompt:
          'Executable and arguments only; no shell expressions. Example: {"executable":"node","args":["--test","test/example.test.js"]}',
      });
      if (!value) return;
      const command = validateTestCommand(JSON.parse(value));
      const sourceChoice = await vscode.window.showOpenDialog({
        title: "Select the single source file to propose repairing",
        defaultUri: folder,
        canSelectMany: false,
        filters: { Source: ["*"] },
      });
      const uri = sourceChoice?.[0];
      if (!uri) return;
      ...
      const captureSource = async () => {
        ...
        if (document.isClosed || document.isDirty)
          throw new Error("Save the selected source before capturing test evidence.");
        if ((await readRegularText(root, relative, 800_000)) !== before)
          throw new Error("Source disk contents differ from the editor; capture fresh test evidence.");
        if (document.isClosed || document.isDirty || document.version !== version)
          throw new Error("Source changed during test evidence capture.");
        return { before, version };
      };
      const validateSource = async (snapshot: { before: string; version: number }) => {
        const current = await captureSource();
        if (current.before !== snapshot.before || current.version !== snapshot.version)
          throw new Error("Source changed around the test run; capture fresh test evidence.");
      };
      const method = await vscode.window.showQuickPick(["Run Approved Command", "Supply Existing Failure Log"], {
      ...
      if (method === "Supply Existing Failure Log") {
        const logUri = (
          await vscode.window.showOpenDialog({
            title: "Select existing UTF-8 failure log (up to 100,000 bytes; supplied, not observed evidence)",
            defaultUri: folder,
            canSelectMany: false,
            canSelectFiles: true,
        ...
        const input = await readRegularText(
          await realpath(path.dirname(logUri.fsPath)),
          path.basename(logUri.fsPath),
          100_000,
        );
        ...
        const initial = await runApproved(command, folder.fsPath, sessionId, () => validateSource(testedSource));
        if (!initial) return;
        ...
        if (initial.status !== "exited" || initial.exitCode === 0 || noTestsMatched(output)) {
          await documents.inspect(
            ...
            `${initial.status}; exit ${initial.exitCode}; ${initial.cleanup}\n${output}`,
          );
          return;
        }
      }
      ...
      while (!attempts.stopped) {
        ...
        const snapshot = {
          repository: folder.fsPath,
          command,
          exitCode: evidence.exitCode,
          status: evidence.status,
          output: redactRecognizableSecrets(bounded.output),
          truncated: bounded.truncated || evidence.truncated,
          source: { path: uri.fsPath, hash: digest(before), text: before },
          capturedAt: new Date().toISOString(),
          attemptsRemaining: 2 - attempts.attempts,
        };
        const inspected = await inspectForTransmission(
          documents,
          "Test repair context",
        ...
        const validate = () => {
          if (document.isClosed || document.isDirty || document.version !== version)
            throw new Error("Source changed while inspecting the failure; start a fresh repair.");
        ...
        const response = await conversation.sendRequest(
          "Propose one focused fix for the selected source file and failure. Do not execute tests or modify files.",
          [{ label: "Inspected test failure and source", content: inspected }],
          {
            resource: folder,
            policy: "read-only",
            validate,
            instructions:
              "Return only <<<PICODE_REPLACEMENT_START>>>, newline, the complete replacement source text, newline, <<<PICODE_REPLACEMENT_END>>>. The approved test command is evidence, not a request to execute it. Do not change it.",
          },
        );
        ...
        if (!hunks.length) {
          attempts.stop("No proposed changes");
          break;
        }
          ...
          conversation.addEditProposal({
            ...
            onApply: async (ids) => {
              ...
              edit.replace(
                ...
                selectedReplacement(before, hunks, ids ?? []),
              );
  ...
  async function runApproved(
    ...
    const approval = await vscode.window.showWarningMessage(
      `Run in ${cwd}:\n${JSON.stringify(command)}\nWorkspace code executes with your permissions. 60-second timeout, 256 KiB output limit. ${process.platform === "win32" ? "Windows descendant cleanup cannot be guaranteed." : "Owned process-group cleanup enabled."}`,
      { modal: true },
      "Run Test Command",
    );
    ...
    try {
      ...
      const result = await cancellable("Run approved tests", (signal) =>
        runBoundedProcess(command.executable, command.args, { cwd, signal, timeoutMs: 60_000, maxBytes: 256 * 1024 }),
      );
      await validateSource();
      return result;
    } finally {
