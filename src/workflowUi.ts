...
export class WorkflowDocuments implements vscode.TextDocumentContentProvider, vscode.Disposable {
  ...
  public provideTextDocumentContent(uri: vscode.Uri): string {
    return this.contents.get(uri.toString()) ?? "This Pi snapshot has expired.";
  }
...
export function isDirtyFile(absolute: string): boolean {
  ...
  return vscode.workspace.textDocuments.some(
    (document) =>
      document.uri.scheme === "file" && document.isDirty && canonical(document.uri.fsPath) === canonical(absolute),
  );
}
...
export function requireTrustedFile(uri?: vscode.Uri): void {
  ...
  if (uri && uri.scheme !== "file")
    throw new Error("This Pi workflow requires file-backed resources; virtual resources are unsupported.");
}
export async function assertRuntimeTarget(runtime: PiRuntimeManager, root: string, sessionId?: string): Promise<void> {
  ...
  if ((await realpath(runtime.currentCwd)) !== (await realpath(root))) {
    throw new Error(
      "The target does not match Pi's active working directory. Explicitly switch the workspace/session first.",
    );
  }
  if (!runtime.currentState.connected) await runtime.ensureStarted(vscode.Uri.file(root));
  if (sessionId !== undefined && runtime.currentState.sessionId !== sessionId)
    throw new Error("The Pi session changed; capture a new snapshot.");
}
    ...
    folders.length === 1
      ...
      : (
          await vscode.window.showQuickPick(
            ...
            { title: "Choose Pi workflow workspace" },
          )
        )?.folder;
...
export async function inspectForTransmission(
  ...
  const action = await vscode.window.showWarningMessage(
    `${warningPrefix}Inspect this local snapshot. Pi history/instructions and later tool reads are separate. Secret warnings are best-effort. Nothing has been sent yet.`,
    "Send Snapshot",
    "Edit / Redact",
  );
  ...
  const secret = await vscode.window.showInputBox({
    title: "Redact snapshot text",
    prompt: "Exact text to replace with [REDACTED]. The edited snapshot will be inspected again before transmission.",
    password: true,
  });
