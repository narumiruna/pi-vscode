...
export interface RequestCheckpoint {
  ...
  readonly status: "completed" | "cancelled" | "process-exit";
...
export class CheckpointHistory {
  ...
  public get values(): readonly RequestCheckpoint[] {
    return this.records;
  }
  public get retainedBytes(): number {
    return this.records.reduce(
      (total, item) =>
        total +
        item.files.reduce((sum, file) => sum + Buffer.byteLength(file.before) + Buffer.byteLength(file.after), 0),
      0,
    );
  }
  public add(input: Omit<RequestCheckpoint, "id">): RequestCheckpoint {
    ...
    record.files = record.files.filter((file) => {
      const before = Buffer.byteLength(file.before);
      const after = Buffer.byteLength(file.after);
      if (before > 2 * 1024 * 1024 || after > 2 * 1024 * 1024 || bytes + before + after > 20 * 1024 * 1024) {
        excluded.push(`${file.path}: retained snapshot limit`);
        return false;
      }
      bytes += before + after;
      return true;
    ...
    if (!this.records.includes(stored)) {
      ...
      stored.exclusions = [
        ...stored.exclusions,
        "History capacity reserved for unresolved recovery snapshots; this request was not retained.",
      ];
    }
    return stored;
  }
  ...
  public assertRestorable(
    id: string,
    selected: readonly string[],
    current: (file: string) => { text?: string; dirty: boolean },
  ): void {
    const record = this.require(id);
    if (!selected.length || new Set(selected).size !== selected.length)
      throw new Error("Select covered checkpoint files.");
    for (const filePath of selected) {
      ...
      const later = this.records
        .slice(this.records.indexOf(record) + 1)
        .some(
          (item) =>
            item.repository === record.repository &&
            item.files.some((other) => other.path === filePath && !other.reverted),
        );
      if (later) throw new Error("Revert newer dependent checkpoints first.");
      const resource = current(filePath);
      if (resource.dirty || resource.text === undefined || digest(resource.text) !== digest(file.after))
        throw new Error("Checkpoint file is stale or has a dirty editor buffer.");
    }
  }
  public clear(): void {
    this.records = [];
